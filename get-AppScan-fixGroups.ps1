...
#try {
   ...
   $issuesUrl = "$BaseUrl/FixGroups/" + $Scope +"/" + $ScanId + "?applyPolicies=None&`$top=100&count=false&" + $filter
...
#echo $issue
    [
PSCustomObject]@{
        applicationId = $Issue.AppId
        scanId        = $ScanId
        issueId       = $issue.Id
        FixGroupType  = $issue.FixGroupType
        FixGroupId    = $issue.Id
        ...
        File          = $issue.File
        Line          = $issue.line
        NIssues       = $issue.NIssues
        NOpenIssues   = $issue.NOpenIssues
